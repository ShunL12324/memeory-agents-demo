...
class WorkflowLogger:
    ...
    def __init__(self, session_id: str = None):
        ...
        self.json_log_file = self.logs_dir / f"workflow_{self.session_id}.json"
        ...
        self._write_log("INFO", "workflow", "Workflow logging started", {
            "session_id": self.session_id,
    ...
    def _write_log(self, level: str, component: str, message: str, data: Dict[str, Any] = None):
    ...
    def _write_markdown_entry(self, timestamp: str, level: str, component: str, message: str, data: Dict[str, Any] = None):
        ...
        with open(self.md_log_file, 'a', encoding='utf-8') as f:
            ...
            if component.endswith("_agent"):
                ...
                self._write_workflow_markdown(f, time_str, level, component, message, data)
    
    def _write_agent_markdown(self, f, time_str: str, level: str, component: str, message: str, data: Dict[str, Any]):
        ...
        if data and "input" in data:
            ...
            if "system_message" in input_data and input_data["system_message"]:
                f.write(f"SYSTEM> {input_data['system_message'][:150]}{'...' if len(input_data['system_message']) > 150 else ''}\n\n")
    ...
    def _write_tool_markdown(self, f, time_str: str, level: str, component: str, message: str, data: Dict[str, Any]):
    ...
    def _write_error_markdown(self, f, time_str: str, component: str, message: str, data: Dict[str, Any]):
    ...
    def _write_workflow_markdown(self, f, time_str: str, level: str, component: str, message: str, data: Dict[str, Any]):
    ...
    def log_workflow_start(self, character_request: str, context: Dict[str, Any] = None):
        """Log workflow start"""
        self._write_log("INFO", "workflow", "Character creation workflow started", {
    ...
    def log_workflow_end(self, status: str, final_state: Dict[str, Any] = None):
        ...
        self._write_log("INFO", "workflow", f"Workflow completed with status: {status}", {
            "final_state": final_state or {}
        })
    
    def log_agent_response(self, agent_name: str, action: str, input_data: Dict[str, Any], response: str, processing_time: float = None):
        """Log agent response"""
        self._write_log("INFO", f"{agent_name}_agent", f"Agent action: {action}", {
            "input": input_data,
    ...
    def log_tool_call(self, agent_name: str, tool_name: str, tool_input: Dict[str, Any], tool_output: str, success: bool = True):
        """Log tool call"""
        level = "INFO" if success else "ERROR"
        self._write_log(level, f"tool_{tool_name}", f"Tool called by {agent_name}", {
            "tool_input": tool_input,
            "tool_output": tool_output[:300] + "..." if len(tool_output) > 300 else tool_output,
            "success": success,
            "agent": agent_name
        })
    
    def log_state_transition(self, from_node: str, to_node: str, state_data: Dict[str, Any]):
        """Log state transition between nodes"""
        self._write_log("INFO", "workflow", f"State transition: {from_node} -> {to_node}", {
            "from_node": from_node,
            "to_node": to_node,
    ...
    def log_task_completion(self, task_id: str, task_name: str, asset_info: Dict[str, Any]):
        """Log task completion"""
        self._write_log("INFO", "task", f"Task completed: {task_id}", {
            "task_id": task_id,
            "task_name": task_name,
    ...
    def log_error(self, component: str, error_message: str, error_data: Dict[str, Any] = None):
        """Log error"""
    ...
    def log_plan_creation(self, plan: Dict[str, Any]):
        ...
        self._write_log("INFO", "planner_agent", f"Plan created with {len(tasks)} tasks", {
            "task_count": len(tasks),
            "task_list": [{"task_id": t.get("task_id"), "task_name": t.get("task_name")} for t in tasks]
        })
    
    def log_todo_file_operation(self, operation: str, file_path: str, result: str):
        """Log todo file operations"""
        self._write_log("INFO", "supervisor_agent", f"Todo file {operation}", {
            "operation": operation,
            "file_path": file_path,
            "result": result
        })
...
def log_agent_response(agent_name: str, action: str, input_data: Dict[str, Any], response: str, processing_time: float = None):
    """Convenience function to log agent response"""
    get_logger().log_agent_response(agent_name, action, input_data, response, processing_time)


def log_tool_call(agent_name: str, tool_name: str, tool_input: Dict[str, Any], tool_output: str, success: bool = True):
